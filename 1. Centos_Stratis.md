...
fstab(자동마운트) 등록
파일시스템 확인 명령어 df-TH 명령어 란 Type 확인
```
...
디스크이름으로 할시 이머전시 모드로 빠지는 경우가 있어 가능하면 UUID로 쓸것
루트로서 /etc/fstab 파일을 편집하고 UUID로 식별되는 파일 시스템 행을 추가
xfs파일 시스템 유형으로 사용 하고 x-systemd.requires=stratisd.service옵션을 추가
